...
public interface HomeRecMovieRepository extends JpaRepository<Movie, Long>, SearchQuerydsl {
    ...
    @Query(value = 
    		"WITH rankeditems AS ( " +
    	            ...
    	            "        ROW_NUMBER() OVER (PARTITION BY '장르' ORDER BY SUM(COALESCE(i.importance_value, 0)) DESC) AS rn " +
    	            "    FROM movies m " +
    	            ...
    	            "        SUM(NVL(i.importance_value, 0)) AS 선호도, " +
    	            "        ROW_NUMBER() OVER (PARTITION BY '국가' ORDER BY SUM(COALESCE(i.importance_value, 0)) DESC) AS rn " +
    	            "    FROM movies m " +
    	            "    JOIN users_movies um ON m.movie_id = um.movie_id " +
    	            ...
    	            "        p.person_name AS 이름, " +
    	            "        SUM(NVL(i.importance_value, 0)) AS 선호도, " +
    	            "        ROW_NUMBER() OVER (PARTITION BY '사람' ORDER BY SUM(COALESCE(i.importance_value, 0)) DESC) AS rn " +
    	            "    FROM movies m " +
    	            "    JOIN users_movies um ON m.movie_id = um.movie_id " +
    	            ...
    	            nativeQuery = true)
    	    List<Object[]> findMoviesByUserId(@Param("userId") Long userIdLong);
}
