...
public interface HomeRecMovieRepository extends JpaRepository<Movie, Long>, SearchQuerydsl {
    ...
    @Query(value = "WITH rankeditems AS (" +
            "    SELECT " +
            "        um.user_id, '장르' AS 타입, g.genre_name AS 이름, " +
            ...
            "    UNION ALL " +
            "    SELECT " +
            "        um.user_id, '국가' AS 타입, c.country_name AS 이름, " +
            "        SUM(NVL(i.importance_value, 0)) AS 선호도, " +
            ...
            "    UNION ALL " +
            "    SELECT " +
            "        um.user_id, '사람' AS 타입, p.person_name AS 이름, " +
            "        SUM(NVL(i.importance_value, 0)) AS 선호도, " +
            ...
            nativeQuery = true)
    List<HomeRecMovieDto> findMoviesByUserId(@Param("userId") Long userId);
}
